    ...
    <div v-if="!isCollapsed" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <label class="text-xs text-gray-600">
        ...
        <select v-model="targetId" class="mt-1 w-full rounded border border-gray-300 bg-white px-2 py-1.5 text-sm">
          <option v-for="target in nodeTargets" :key="target.id" :value="target.id">{{ target.label }}</option>
        </select>
      </label>
...
const selectedTarget = computed(() => {
  return props.nodeTargets.find(target => target.id === targetId.value) ?? null;
});
...
watch(() => props.nodeTargets, (next) => {
  ...
  if (!next.some(target => target.id === targetId.value)) {
    targetId.value = next[0].id;
  }
}, { immediate: true });
...
function submit() {
  if (!targetId.value || !selectedTarget.value) return;
  ...
  emit("submit", {
    ...
    target_region: selectedTarget.value.region ?? null,
    infra_name: selectedTarget.value.infraName ?? null,
