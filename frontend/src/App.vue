<template>
  <div class="max-w-[1100px] mx-auto px-5 py-5">
    ...
    <div class="flex gap-3 items-center mb-4 flex-wrap">
      ...
      <!-- WS status badge -->
      <span class="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border font-medium" :class="wsStatus === 'connected' ? 'border-green-500 text-green-700'
        : wsStatus === 'connecting' ? 'border-yellow-400 text-yellow-600'
          : 'border-gray-300 text-gray-400'">
        <span class="w-1.5 h-1.5 rounded-full" :class="wsStatus === 'connected' ? 'bg-green-500'
    ...
    <!-- Empty state -->
    <div v-else-if="regions.length === 0"
      class="p-4 border border-dashed border-gray-300 rounded-lg text-sm text-gray-600">
      No regions reporting yet. Start nodes like:
      <pre
        class="mt-2 bg-gray-50 border border-gray-200 rounded p-3 text-xs overflow-x-auto">{{ exampleCommands }}</pre>
    </div>
    ...
    <div v-else>
      ...
      <div class="grid gap-4 items-start" style="grid-template-columns: repeat(auto-fit, minmax(320px, 1fr));">
        <div v-for="r in regions" :key="r.name" class="border rounded-xl p-4 shadow-sm bg-white transition-colors"
          ...
          <div class="flex items-start justify-between gap-3 mb-3">
            <div>
              ...
              <div class="flex items-center gap-2 mt-1">
                <span class="text-[11px] px-1.5 py-0.5 rounded border font-medium uppercase tracking-wide" :class="r.regionType === 'urban' ? 'border-purple-300 text-purple-700 bg-purple-50'
          ...
          <div v-if="r.sites && r.sites.length" class="mt-3">
            <button @click="toggleSites(r.name)"
...
async function tryConnect(endpoint) {
  ...
  try {
    ...
    if (handshake.status !== "success") {
      ws.close();
      ws = null;
      console.log(`Endpoint (${endpoint}) did not receive notification of success.`)
      return;
    }
