...
class PatchAction:
    ...
    def apply(self, target: dict, rewind: bool):
...
class PatchAddKey(PatchAction):
    ...
    def apply(self, target: dict, rewind: bool):
        ...
        if rewind:
            had_key = None if name not in trg else trg[name]
        trg[name] = self.value
        if rewind:
            if had_key is None:
                return PatchDeleteKey(self.path)
            else:
                return PatchDeleteKey(self.pa)
@dataclass
class PatchDeleteKey(PatchAction):
    ...
    def apply(self, target: dict, rewind: bool):
        ...
        del trg[name]
        if rewind:
            return PatchAddKey(self.path, ov)
...
class PatchEditField(PatchAction):
    ...
    def apply(self, target: dict, rewind: bool):
        ...
        trg[name] = self.value
        if rewind:
            return PatchEditField(self.path, ov)
...
class PatchArray(PatchAction):
    ...
    def __init__(self, path: str, end: int, index_sets: dict[int, Any]):
        ...
        if len(index_sets) > 0 and isinstance(list(index_sets.keys())[0], str):
            # print("YES")
    ...
    def apply(self, target, rewind: bool):
        trg, name = _locate_patch(self.path, target)
        if rewind:
            o_end = len(trg[name])
            o_is = {}
            if o_end > self.end:
                for i in range(self.end, o_end):
                    o_is[i] = trg[name][i]
        trg[name] = trg[name][:self.end]
        for idx, val in self.index_sets.items():
            if idx >= len(trg[name]):
                trg[name].insert(idx, val)
                # NOTE: There is no special rewind case here since it will be
                # truncated away.
            else:
                if rewind:
                    o_is[idx] = trg[name][idx]
                trg[name][idx] = val
        if rewind:
            return PatchArray(self.path, o_end, o_is)
...
class Patch:
    ...
    @staticmethod
    def diff(original: dict, updated: dict) -> Patch:
        return generate_patch(original, updated)
    ...
    def apply_inplace(self, target: dict, get_rewind: bool = False) -> Optional[Patch]:
        rewinder = []
        for action in self.actions:
            o = action.apply(target, get_rewind)
            if get_rewind:
                rewinder.append(o)
        if get_rewind:
            return Patch(rewinder)
